...
| **Test (Compose)** | `/opt/test` → https://test.orion13.us (`upanel-edge` → `upanel-test-nginx`; host **TEST_HTTP_PORT** often **:8085** for IP) | [SERVER_SETUP.md](SERVER_SETUP.md#test-environment-opttest--httpstestorion13us) · `scripts/contabo/deploy-test-on-server.sh` |
...
On Contabo production today, **Cloudflare Flexible SSL** terminates HTTPS at the edge and the origin listens on **:80** (see [CLOUDFLARE_DNS_SETUP.md](CLOUDFLARE_DNS_SETUP.md)). The test hostname uses the same origin port; prod nginx proxies over Docker network **`upanel-edge`** to `upanel-test-nginx:80`. Host `TEST_HTTP_PORT` (default `:8080`, often `:8085`) is for direct IP access only.
...
Test: Host test.orion13.us → prod nginx → upanel-test-nginx:80 (Docker DNS on upanel-edge)
      Direct IP: SERVER_IP:${TEST_HTTP_PORT} → upanel-test nginx (host publish)
```
