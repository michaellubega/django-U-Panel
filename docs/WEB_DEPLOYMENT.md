...
3. **Builds Flutter web** (or `USE_COMMITTED_WEB=1` to bake committed `website/app` when it already contains `KIU-QAAT`) and refuses to deploy if `main.dart.js` lacks `KIU-QAAT`
...
```bash
curl -sS https://test.orion13.us/app/version.json   # expect build_number >= 14
...
Demo logins: `KIU-VC01` … `KIU-HOD1` / `qaat@kiu`. Admin and QA also land on the QAAT Dashboard home (ops on Attendance).
