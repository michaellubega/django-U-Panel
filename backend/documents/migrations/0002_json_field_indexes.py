...
class Migration(migrations.Migration):
    ...
    operations = [
        ...
        *(
        ...
        migrations.RunSQL(
            """
            CREATE INDEX CONCURRENTLY IF NOT EXISTS doc_data_active_sessions_idx
            ...
            """,
            reverse_sql=(
                "DROP INDEX IF EXISTS doc_data_active_sessions_idx;"
            ),
        ),
        # Partial index: pending check-in attempts awaiting session link.
        migrations.RunSQL(
            """
            CREATE INDEX CONCURRENTLY IF NOT EXISTS doc_data_awaiting_claims_idx
            ON documents_apidocument (
                (data ->> 'sessionCodeRaw'),
                (data ->> 'status')
            )
            ...
            """,
            reverse_sql=(
                "DROP INDEX IF EXISTS doc_data_awaiting_claims_idx;"
            ),
        ),
        # Partial index: attendance records by session (roll display).
        migrations.RunSQL(
            """
            CREATE INDEX CONCURRENTLY IF NOT EXISTS doc_data_records_session_idx
            ON documents_apidocument (
                (data ->> 'sessionId')
            )
            ...
            """,
            reverse_sql=(
                "DROP INDEX IF EXISTS doc_data_records_session_idx;"
            ),
        ),
    ]
